...
-- worlwide_gross_income - 3724 NULL VALUES
...
GROUP BY country
HAVING year=2019;
-- USA AND INDIA HAS PRODUCED 1007 NUMBER  OF MOVIES IN 2019
...
-- Type your code below:
SELECT COUNT(movie_id) AS NUMBER_OF_MOVIES , genre
FROM genre as g 
INNER JOIN 
movie as m 
ON g.movie_id = m.id
...
WHERE year = 2017 AND MONTH(date_published)=03
...
-- Type your code below:
SELECT COUNT(total_votes) AS total_votes , country
from movie as m 
INNER JOIN ratings as r 
on m.id = r.movie_id
...
-- Type your code below:
WITH  genre_summ as
(SELECT COUNT(g.movie_id) as movie_count , genre, g.movie_id
FROM movie as m 
...
INNER JOIN ratings AS r
ON g.movie_id =r.movie_id,genre_summ
WHERE 
        avg_rating >8 AND
                   g.genre in  (genre_summ.genre)
...
WHEN avg_rating BETWEEN 5 AND 7 THEN "One_time_watch movies "
...
SELECT * FROM HIGHEST_GROSSING_MOVIE 
WHERE movie_rank<=5;
...
-- Type your code below:
WITH prod_comp AS 
(SELECT production_company,COUNT(id) as movie_count ,languages, 
